...
use vm_core::{
    ...
    code_blocks::{get_span_op_group_count, CodeBlock, CodeBlockType},
...
fn hash_memoization_control_blocks() {
    ...
    let (_, final_state) = hasher.hash_control_block(CodeBlockType::JOIN, h1, h2, expected_hash, &mut lookups);
    ...
    let (addr, final_state) = hasher.hash_control_block(CodeBlockType::SPLIT, h1, h2, expected_hash, &mut lookups);
